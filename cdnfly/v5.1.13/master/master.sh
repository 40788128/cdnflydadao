...
if [[ $VER == "" ]]; then
  # 获取最新版本
  echo "获取最新版..."
  latest_version=`curl -s 'https://github.com/40788128/cdnflydadao/raw/main/cdnfly/v5.1.13/master/upgrades.json' | grep -Po 'v\d+\.\d+.\d+' || true`
...
sed -i "s/https:\/\/artifacts.elastic.co\/downloads\/elasticsearch\/elasticsearch-7.6.1-x86_64.rpm/g" install.sh
sed -i "s/https:\/\/artifacts.elastic.co\/downloads\/elasticsearch\/elasticsearch-7.6.1-amd64.deb/g" install.sh
chmod +x install.sh
./install.sh $@
